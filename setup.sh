...
# setup.sh - Script to install and configure the Streamlit Speedtest Dashboard.

# --- Configuration Variables ---
APP_USER="speedtest-app"
APP_GROUP="speedtest-app"
APP_DIR="/opt/speedtest-dashboard"
VENV_DIR="$APP_DIR/venv"
START_SCRIPT_NAME="start_dashboard.sh"
SERVICE_FILE_NAME="speedtest-dashboard.service"
SPEEDTEST_PY_FILE="speedtest.py"
SPEEDTEST_DASH_PY_FILE="speedtest_dash.py"
...
print_info "Starting Streamlit Speedtest Dashboard setup..."

# --- 1. Create User and Group ---
print_info "Creating user '$APP_USER' and group '$APP_GROUP'..."
if ! getent group "$APP_GROUP" > /dev/null; then
    groupadd "$APP_GROUP" || print_error "Failed to create group '$APP_GROUP'."
    print_success "Group '$APP_GROUP' created."
else
    print_info "Group '$APP_GROUP' already exists."
fi

if ! id -u "$APP_USER" > /dev/null 2>&1; then
    useradd -r -s /bin/false -g "$APP_GROUP" -d "$APP_DIR" "$APP_USER" || print_error "Failed to create user '$APP_USER'."
...
chown -R "$APP_USER":"$APP_GROUP" "$APP_DIR" || print_error "Failed to set ownership for '$APP_DIR'."
chmod 755 "$APP_DIR" || print_error "Failed to set permissions for '$APP_DIR'."
print_success "Directory '$APP_DIR' created and permissions set."
...
if ! dpkg -s python3-venv >/dev/null 2>&1; then
    ...
    apt-get update && apt-get install -y python3-venv || print_error "Failed to install python3-venv."
...
print_info "Installing Python packages (streamlit, pandas, speedtest-cli) into virtual environment..."
"$VENV_DIR/bin/pip" install streamlit pandas speedtest-cli || print_error "Failed to install Python packages."
print_success "Python packages installed."

# --- 5. Place Application Files ---
print_info "Placing application files into '$APP_DIR'..."

# Write speedtest.py using a here-document
cat << 'EOF_SPEEDTEST_PY' > "$APP_DIR/$SPEEDTEST_PY_FILE" || print_error "Failed to write $SPEEDTEST_PY_FILE."
#!/usr/bin/python3
###!/usr/bin/env python
# -*- coding: utf-8 -*-
...
if __name__ == '__main__':
    main()
EOF_SPEEDTEST_PY
chown "$APP_USER":"$APP_GROUP" "$APP_DIR/$SPEEDTEST_PY_FILE" || print_error "Failed to set ownership for $SPEEDTEST_PY_FILE."
chmod 644 "$APP_DIR/$SPEEDTEST_PY_FILE" || print_error "Failed to set permissions for $SPEEDTEST_PY_FILE."
print_success "$SPEEDTEST_PY_FILE placed."

# Write speedtest_dash.py using a here-document (UPDATED WITH PERSISTENCE)
cat << 'EOF_SPEEDTEST_DASH_PY' > "$APP_DIR/$SPEEDTEST_DASH_PY_FILE" || print_error "Failed to write $SPEEDTEST_DASH_PY_FILE."
...
time.sleep(60)
st.rerun()
EOF_SPEEDTEST_DASH_PY
chown "$APP_USER":"$APP_GROUP" "$APP_DIR/$SPEEDTEST_DASH_PY_FILE" || print_error "Failed to set ownership for $SPEEDTEST_DASH_PY_FILE."
chmod 644 "$APP_DIR/$SPEEDTEST_DASH_PY_FILE" || print_error "Failed to set permissions for $SPEEDTEST_DASH_PY_FILE."
print_success "$SPEEDTEST_DASH_PY_FILE placed."

# Write start_dashboard.sh using a here-document
cat << 'EOF_START_SCRIPT' > "$APP_DIR/$START_SCRIPT_NAME" || print_error "Failed to write $START_SCRIPT_NAME."
#!/bin/bash
# Script to start the Streamlit Speedtest Dashboard service.

# Set the working directory for the script.
cd /opt/speedtest-dashboard || { echo "Failed to change directory to /opt/speedtest-dashboard"; exit 1; }

# IMPORTANT: Set the HOME environment variable to a temporary directory.
# This tells Streamlit to store its user-specific files (like .streamlit/secrets.toml
# and machine ID files) within this temporary location instead of /home/speedtest-app.
# This works because PrivateTmp=true in the systemd service ensures /tmp is writable
# and private to this service instance.
export HOME="/tmp/streamlit_user_data"
mkdir -p "$HOME/.streamlit" # Ensure the .streamlit directory exists within the temporary home

# Path to your virtual environment
VENV_PATH="./venv"

# Path to your Streamlit application script
APP_SCRIPT="speedtest_dash.py"

# Activate the virtual environment.
source "$VENV_PATH/bin/activate" || { echo "Failed to activate virtual environment"; exit 1; }

# Execute the Streamlit application.
exec streamlit run "$APP_SCRIPT" \
    --server.port 8503 \
    --server.headless true \
    --server.enableCORS false \
    --server.enableXsrfProtection false
EOF_START_SCRIPT
chown "$APP_USER":"$APP_GROUP" "$APP_DIR/$START_SCRIPT_NAME" || print_error "Failed to set ownership for $START_SCRIPT_NAME."
chmod 755 "$APP_DIR/$START_SCRIPT_NAME" || print_error "Failed to set permissions for $START_SCRIPT_NAME."
print_success "$START_SCRIPT_NAME placed and made executable."

# Write speedtest-dashboard.service using a here-document
cat << 'EOF_SERVICE_FILE' > "/etc/systemd/system/$SERVICE_FILE_NAME" || print_error "Failed to write $SERVICE_FILE_NAME."
[Unit]
Description=Streamlit Speedtest Dashboard
...
# --- Execution ---
# Set the working directory to where your app files are located.
# This is crucial for the wrapper script to find its components and for Streamlit
# to find speedtest_dash.py.
WorkingDirectory=/opt/speedtest-dashboard

# Command to start the Streamlit application via the wrapper script.
ExecStart=/opt/speedtest-dashboard/start_dashboard.sh
...
# IMPORTANT FIX: Re-enabling ProtectHome=true.
# The 'HOME' environment variable is now set in start_dashboard.sh to a
# temporary location within /tmp (e.g., /tmp/streamlit_user_data).
# Since PrivateTmp=true ensures /tmp is writable and private for this service,
# Streamlit will write its files there, avoiding permission issues in /home.
ProtectHome=true
...
EOF_SERVICE_FILE
chmod 644 "/etc/systemd/system/$SERVICE_FILE_NAME" || print_error "Failed to set permissions for $SERVICE_FILE_NAME."
print_success "$SERVICE_FILE_NAME placed in systemd."
...
systemctl start "$SERVICE_FILE_NAME" || print_error "Failed to start service."
print_success "Service '$SERVICE_FILE_NAME' enabled and started."

print_info "Waiting a few seconds for the service to fully initialize..."
sleep 5

# --- 7. Verify Service Status ---
print_info "Checking service status..."
if systemctl is-active --quiet "$SERVICE_FILE_NAME"; then
    print_success "Streamlit Speedtest Dashboard service is running successfully!"
    print_info "You should now be able to access the dashboard in your browser on port 8503 (e.g., http://<your-server-ip>:8503)"
    print_info "To check the service logs, run: sudo journalctl -u $SERVICE_FILE_NAME -f"
else
    print_error "Streamlit Speedtest Dashboard service failed to start."
    print_info "Please check the logs for more details: sudo journalctl -u $SERVICE_FILE_NAME -f"
fi
